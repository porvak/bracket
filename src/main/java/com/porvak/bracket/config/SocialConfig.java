...
import com.porvak.bracket.social.connect.AccountSignInAdapter;
import com.porvak.bracket.social.connect.TwitterConnectInterceptor;
import com.porvak.bracket.social.members.ProfilePictureService;
...
public class SocialConfig {
	 ...
	 * When support for a new provider is added to Greenhouse, simply register the corresponding {@link org.springframework.social.connect.ConnectionFactory} here.
	 ...
	 * This bean is defined as a scoped-proxy so it can be serialized in support of {@link org.springframework.social.connect.web.ProviderSignInAttempt provier sign-in attempts}.
	 */
	@Bean
...
//		registry.addConnectionFactory(new TripItConnectionFactory(environment.getProperty("tripit.consumerKey"), environment.getProperty("tripit.consumerSecret")));
	 ...
	 * The returned repository encrypts the data using the configured {@link org.springframework.security.crypto.encrypt.TextEncryptor}.
	 */
	@Bean
	 ...
	 * Since it is a scoped-proxy, references to this bean MAY be injected at application startup time.
	 * The target is an authorized {@link org.springframework.social.twitter.api.Twitter} instance if the current user has connected his or her account with a Twitter account.
	 ...
	 *
	 * @param profilePictureService needed by the {to make the user's Facebook profile picture their Greenhouse profile picture.
	 */
//	@Bean
