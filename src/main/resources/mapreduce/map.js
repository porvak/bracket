        ...
        for (var roundId in rounds) {
            ...
            if (pointsForRound > 0) {
                ...
                for (var gameId in games) {
                    ...
                    // Set the mapping key
                    var gameKey = { userId : this.userId, poolId : this.poolId, tournamentId : this.tournamentId, roundId : dbRoundId };
                    ...
                    if (pickedTeam && loserList.indexOf(pickedTeam.teamId) != -1) {
                        weWon = false;
                    }
                    ...
                    if (weWon == null) {
                        ...
                        emit(gameKey, { score : 0, pointsCounted : 0, maxPointsPossible: maxPossible });
                    } else if (weWon) {
                        // If we won, take the points
                        emit(gameKey, { score : pointsForRound, pointsCounted : pointsForRound, maxPointsPossible: maxPossible });
                    } else {
                        // If we lost, score nothing
                        emit(gameKey, { score : 0, pointsCounted : pointsForRound, maxPointsPossible: maxPossible });
                    }
                }
            }
        }
